  ...
  return (
    <div className="bg-white p-8 rounded-lg shadow-lg max-w-3xl mx-auto">
      <div className="flex items-center space-x-3 mb-8">
        <Search className="h-6 w-6 text-indigo-600" />
        <h2 className="text-3xl font-semibold text-gray-800">Verify Certificates</h2>
      </div>
      ...
      {result && (
        <div className={`mt-8 p-6 rounded-lg ${result.isValid ? 'bg-green-50' : 'bg-red-50'}`}>
          <div className="flex items-center space-x-3">
            ...
            <p className={`text-lg font-semibold ${result.isValid ? 'text-green-700' : 'text-red-700'}`}>
              {result.message}
            </p>
          </div>
          ...
          {result.certificates && result.certificates.length > 0 && (
            <div className="mt-6 space-y-6">
              {result.certificates.map((certificate) => (
                <div key={certificate.id} className="space-y-4 bg-white p-6 rounded-lg shadow-sm">
                  <p><strong>Student Name:</strong> {certificate.studentName}</p>
                  <p><strong>Roll Number:</strong> {certificate.rollNumber}</p>
                  <p><strong>Institution:</strong> {certificate.institution}</p>
                  <p><strong>Course:</strong> {certificate.course}</p>
